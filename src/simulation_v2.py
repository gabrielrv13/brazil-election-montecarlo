...
def graficos(df1, df2, trace, pv, p2v, p2t, info_lim_1t, info_matchups, info_indecisos=None):
    ...
    # Outer arc labels — only for segments large enough to label cleanly
    outer_labels = [
    ...
    for idx_seg, text, col, min_angle in outer_labels:
        ...
        rad = np.radians(deg)
        # Push label further out for near-top segments to avoid overlap with arc
        r_label = R_OUT + (0.22 if 60 < deg < 120 else 0.16)
    ...
    for y, cand in enumerate(cands_rej_rev):
        ...
        # Labels inside/outside bar depending on value
        label_x  = min(rej - 1.0, 68)
        label_ha = 'right' if rej > 20 else 'left'
        ax_rej.text(label_x, y + 0.26,
                    f"{rej:.0f}%  →  teto {teto:.0f}%",
                    ha=label_ha, va='center', fontsize=8.5,
                    color='white' if rej > 20 else '#444444', fontweight='bold')
        ax_rej.text(label_x, y - 0.10,
                    status, ha=label_ha, va='center',
                    fontsize=8, color='white' if rej > 20 else bar_color)
