...
def construir_modelo():
    ...
    with pm.Model() as modelo:
        ...
        trace = pm.sample(draws=10_000, tune=2_000, chains=4,
...
def simular_primeiro_turno():
    ...
    for _ in range(N_SIM):
        v = np.maximum(np.random.normal(VOTOS_MEDIA, DESVIO), 0)
        vn = v / v.sum() * 100
        val = vn[:3]; valn = val / val.sum() * 100
        rows.append({
            'Lula': vn[0], 'Flávio': vn[1], 'Outros': vn[2], 'Brancos': vn[3],
            'Lula_val': valn[0], 'Flávio_val': valn[1], 'Outros_val': valn[2],
            'vencedor': CANDIDATOS[np.argmax(vn[:3])],
            'tem_2turno': valn.max() < 50,
        })
...
def simular_segundo_turno():
    ...
    Transferência de votos de 'Outros':
      40 % -> Lula | 35 % -> Flávio | 25 % -> brancos (descartados)
    """
    print("\n[3/4] Simulando 2o turno (Lula vs Flávio)...")
    rows = []
    for _ in range(N_SIM):
        bl = np.random.normal(35, 2.5); bf = np.random.normal(29, 2.5)
        ot = np.random.normal(21, 2.5)
        tl = np.random.normal(0.40, 0.05); tf = np.random.normal(0.35, 0.05)
        vl = max(bl + ot * tl, 0); vf = max(bf + ot * tf, 0)
        tot = vl + vf
        l2, f2 = vl/tot*100, vf/tot*100
        rows.append({'Lula_2T': l2, 'Flávio_2T': f2,
                     'vencedor_2T': 'Lula' if l2 > f2 else 'Flávio Bolsonaro',
                     'diferenca': abs(l2 - f2)})
    df = pd.DataFrame(rows)
...
def relatorio(df1, df2):
    ...
    for c, p in p2v.items(): print(f"  {c:22s} {p:.2f}%")
...
def graficos(df1, df2, trace, pv, p2v, p2t):
    ...
    for i, col in enumerate(['Lula','Flávio','Outros','Brancos']):
        ax.hist(df1[col], bins=60, alpha=0.6, label=CANDIDATOS[i],
    ...
    ax.set_yticks(range(len(ps))); ax.set_yticklabels(ps.index)
    ax.set_title('Prob. Vitória\n1º Turno', fontweight='bold')
    for i, v in enumerate(ps.values): ax.text(v+.5, i, f'{v:.1f}%', va='center', fontsize=9)
    ...
    ax.set_title('Lula — Votos Válidos', fontweight='bold'); ax.legend(fontsize=8); ax.grid(alpha=0.3)

    # Flávio válidos
    ax = fig.add_subplot(gs[1, 1])
    ax.hist(df1['Flávio_val'], bins=60, color='#3498db', alpha=0.7, edgecolor='black', lw=0.3)
    ax.axvline(50, color='red', ls='--', lw=2, label='50%')
    ax.axvline(df1['Flávio_val'].mean(), color='darkblue', lw=2, label=f'Média {df1["Flávio_val"].mean():.1f}%')
    ax.set_title('Flávio — Votos Válidos', fontweight='bold'); ax.legend(fontsize=8); ax.grid(alpha=0.3)
    ...
    ax.set_title('Lula — 2º Turno', fontweight='bold'); ax.legend(fontsize=8); ax.grid(alpha=0.3)

    # Flávio 2T
    ax = fig.add_subplot(gs[2, 1])
    ax.hist(df2['Flávio_2T'], bins=60, color='#3498db', alpha=0.7, edgecolor='black', lw=0.3)
    ax.axvline(50, color='red', ls='--', lw=2)
    ax.axvline(df2['Flávio_2T'].mean(), color='darkblue', lw=2, label=f'Média {df2["Flávio_2T"].mean():.1f}%')
    ax.set_title('Flávio — 2º Turno', fontweight='bold'); ax.legend(fontsize=8); ax.grid(alpha=0.3)

    # barras prob 2T
    ax = fig.add_subplot(gs[2, 2])
    p2s = p2v.sort_values()
    ax.barh(range(len(p2s)), p2s.values, color=['#3498db','#e74c3c'])
    ax.set_yticks(range(len(p2s))); ax.set_yticklabels(p2s.index)
    ax.set_title('Prob. Vitória\n2º Turno', fontweight='bold')
    for i, v in enumerate(p2s.values): ax.text(v+.5, i, f'{v:.1f}%', va='center', fontsize=9)
    ax.grid(alpha=0.3, axis='x')
